...
import os
from PySide2.QtCore import QDir, QFileInfo, QStandardPaths
...
def abspath(path):  
  ...
  @return  unicode
  """
  return QFileInfo(path).absoluteFilePath() if path else ""
...
DESKTOP = (QStandardPaths.standardLocations(QStandardPaths.DataLocation)
    or os.path.join(HOME, 'Desktop'))

SYSTEMDRIVE = os.environ.get('windir') or r"C:"
WINDIR = os.environ.get('WinDir') or os.path.join(SYSTEMDRIVE, r"Windows")
PROGRAMFILES = os.environ.get('ProgramFiles') or os.path.join(SYSTEMDRIVE, r"Program Files")
PROGRAMFILESx86 = os.environ.get('ProgramFiles(x86)') or PROGRAMFILES # try x86 first
COMMONPROGRAMFILES = os.environ.get('CommonProgramFiles') or os.path.join(PROGRAMFILES, r"Common Files")
COMMONPROGRAMFILESx86 = os.environ.get('CommonProgramFiles(x86)') or os.path.join(PROGRAMFILESx86, r"Common Files")
APPDATA = os.environ.get('AppData') or os.path.join(HOME, r"AppData\Roaming")
LOCALAPPDATA = os.environ.get('LocalAppData') or os.path.join(HOME, r"AppData\Local")
SYSTEM32 = os.path.join(WINDIR, 'System32')

def prepend_path(path):
  if path:
    try: os.environ['PATH'] = path + os.pathsep + os.environ['PATH']
    except (OSError, UnicodeDecodeError) as e: dwarn(e)
