...
def dprint(msg, *args):
    ...
    if args:
        if class_:
            safeprint("%s:%s:%s:" % (file_, class_, func),
        ...
        if class_:
            safeprint("%s:%s:%s:" %
                      (file_, class_, func), msg, file=sys.stderr)
        else:
...
def dwarn(msg, *args):
    ...
    if args:
        if class_:
            safeprint(beg + "%s:%s:%s:" % (file_, class_, func),
                      msg, args, end, file=sys.stderr)
        else:
            safeprint(beg + "%s:%s:" % (file_, func),
                      msg, args, end, file=sys.stderr)
    else:
        if class_:
            safeprint(beg + "%s:%s:%s:" %
                      (file_, class_, func), msg, end, file=sys.stderr)
        else:
            safeprint(beg + "%s:%s:" %
                      (file_, func), msg, end, file=sys.stderr)
...
def debugmethod(f):
    ...
    def ret(*args, **kwargs):
        ...
        if class_:
            safeprint("%s:%s:%s: enter" %
                      (file_, class_, func), file=sys.stderr)
        else:
            safeprint("%s:%s: enter" % (file_, func), file=sys.stderr)
