...
if skos.WIN:
            ...
            if hwnd:
                ...
                ret = not skwinapi.ChangeWindowMessageFilterEx or all(
                    skwinapi.ChangeWindowMessageFilterEx(
                        hwnd, it, skwinapi.MSGFLT_ALLOW, 0)
                    for it in wms)
            else:
                # http://answers.microsoft.com/en-us/windows/forum/windows_7-files/file-drag-n-drop-via-wmdropfiles-doesnt-work-in/d172ed8c-1a5b-e011-8dfc-68b599b31bf5?msgId=2e77dbec-495b-e011-8dfc-68b599b31bf5
    ...
    def hwnd_from_wid(winId):
        ...
        Used to convert PySide2 WId to HWND.
        This is a PySide2 specific issue. In PyQt, WId is the same as HWND
    ...
    def create_process(path, params=None, environ=None, unicode_environ=True, suspended=False, complete=False):
        ...
        if environ:
            ...
            if not unicode_environ:
                ...
                try:
                    environ = {str(k): str(v) for k, v in e.items()}
        ...
        try:
            ...
            if environ:
                environ = "\0".join("%s=%s" % (k, v)
                                    for k, v in environ.items()) + "\0"
    ...
    def minimize_window(hwnd):
        ...
        """
        try:
            return bool(win32gui.ShowWindow(hwnd, win32con.SW_MINIMIZE))
        except:
            return False
    ...
    def set_window_menu(hwnd, hmenu):
        ...
        """
        try:
            return bool(win32gui.SetMenu(hwnd, hmenu))
        except:
            return False
    ...
    def get_content_size(hwnd):
        ...
        """
        try:
            # only return bottom and right
            return win32gui.GetClientRect(hwnd)[2:]
        except:
            return 0, 0
    ...
    def get_window_text(hwnd, size=256):
        ...
        try:
            size = 1 + \
                skwinapi.SendMessageW(hwnd, win32con.WM_GETTEXTLENGTH, 0, 0)
            buf = ctypes.create_unicode_buffer(size)
            size = skwinapi.SendMessageW(
                hwnd, win32con.WM_GETTEXT, size, ctypes.addressof(buf))
            if size > 0:
                return buf.value
        except:
            pass
    ...
    def set_window_text(hwnd, text):
        ...
        try:
            ...
            size = skwinapi.SendMessageW(
                hwnd, win32con.WM_SETTEXT, 0, ctypes.addressof(buf))
            return size > 0
        except:
            return False
    ...
    def set_active_window(hwnd):
        ...
        """
        try:
            return bool(win32gui.SetActiveWindow(hwnd))
        except:
            return False
    ...
    def get_first_window(hwnd):
        ...
        """
        try:
            return win32gui.GetWindow(hwnd, win32con.GW_HWNDFIRST)
        except:
            return 0
    ...
    def is_taskbar_autohide(align='b'):
        ...
        try:
            return bool(skwinapi.SHAppBarMessage(
    ...
    def set_display_resolution(size, display=None):
        ...
        try:
            ...
            display.PelsWidth, display.PelsHeight = size
            # 0 means do change
            return win32con.DISP_CHANGE_SUCCESSFUL == win32api.ChangeDisplaySettings(display, 0)
        except Exception as e:
            dwarn(e)
            return False
    ...
    def set_file_hidden(path):  # str -> bool
        try:
            bool(win32api.SetFileAttributes(
                path, win32con.FILE_ATTRIBUTE_HIDDEN))
        except:
            return false
    ...
    def set_file_readonly(path):  # str -> bool
        try:
            bool(win32api.SetFileAttributes(
                path, win32con.FILE_ATTRIBUTE_READONLY))
        except:
            return false
    ...
    def install_msi_admin(path):  # unicode -> bool
        try:
            return elevate('msiexec.exe', ('/i', path))
        except Exception as e:
            dwarn(e)
            return False
...
if __name__ == '__main__':
    path = r"D:\Library\Desktop\GalGame\「虜ノ契」.lnk"
