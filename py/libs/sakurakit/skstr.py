...
def multireplacer(table, flags=0, escape=False, prefix=None, suffix=None):
  ...
  @return  fn(text)
  """
  it = iter(table.keys()) if escape else (re.escape(k) for k in table.keys())
...
def urlencode(text, safe=''):
  ...
  """
  return urllib.parse.quote(text, safe=safe) # to percentage encoding
...
def urldecode(text):
  ...
  @return  str not unicode
  """
  text = urllib.parse.unquote(text) # from percentage encoding
