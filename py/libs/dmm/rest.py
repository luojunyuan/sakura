...
class RestApi(object):
  ...
  def _makeurl(self, **params):
    ...
    request = ["%s=%s" % (k, urllib.parse.quote(self._encodeparam(v)))
        for k,v in sorted(params.items())] # sorted is needed to make the URL unique for caching
  ...
  def _fetch(self, url):
    ...
    handler = urllib.request.HTTPHandler(debuglevel=self.debug)
...
if __name__ == '__main__':
  ...
  t = "幻創のイデア "
  t = "神咒神威神楽"
  t = "528lih5035" # product_id
  t = "000_029pcg" # content_id
  t = '1196wp111' # cid
  t = '魔王はじめました'
  t = 'hawkeye_0005'
  dmm.debug = 1
