...
class SearchApi(object):
  ...
  _rx_first_title = re.compile(r'<p class="sellst3">･<strong>([^<]*?)</strong></p>')
  _rx_first_id = re.compile(r'prdcode=([0-9]+?)">消す</a>')
  _rx_year = re.compile(r'<strong>([0-9]{4}) 年</strong>')
  ...
  def _iterparse(self, h):
    ...
    """
    try:
      start = h.find("検索結果（タイトル）") # int
      stop = h.find("ブランド別製品リスト") # int
