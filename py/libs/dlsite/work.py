...
class WorkApi(object):
  ...
  _re_fields = (
    ...
    ('homepage', re.compile(r'ホームページ.*?href="([^"]+?)"', re.DOTALL)),
  )
  ...
  def _parseecchi(self, h):
    ...
    @return  bool
    """
    return 'class="work_genre">全年齢' not in h
  ...
  def _parsefilesize(self, h):
    ...
    if m:
      try: return int(float(m.group(1)) * 1024 * 1024 * 1024)
      except: pass
  ...
  _rx_reviewcount = re.compile(r'review_work_count">([0-9]+)件')
  ...
  _rx_artist = __makecreatorxp('原画')
  _rx_writer = __makecreatorxp('シナリオ')
  _rx_musician = __makecreatorxp('音楽')
