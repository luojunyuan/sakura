...
class ArticleApi(object):
  ...
  def _parseecchi(self, h):
    ...
    if m:
      return "18禁" in m.group(1)
    return False

  _rx_doujin = __maketablerx("種別")
  def _parsedoujin(self, h):
    ...
    if m:
      return "同人" in m.group(1)
    return False

  _rx_comment = __maketablerx("コメント")
  ...
  _rx_brand = __maketablerx("(?:メーカー|サークル)")
  ...
  _rx_artists = __maketablerx("(?:原画|主な作家)")
  ...
  _rx_price = re.compile(r'価格：<span class="bold">([0-9,]+)</span><b>円</b>')
