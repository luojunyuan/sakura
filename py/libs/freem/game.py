...
class GameApi(object):
  ...
  def _parse(self, h):
    ...
    if title:
      ...
      bl = 'ＢＬゲーム' in h
      otome = bl or '女性向' in h or '乙女ゲーム' in h
      ecchi = '全年齢' in h
      ret = {
        'title': title, # unicode
        'otome': otome, # bool
  ...
  _re_title = re.compile(r'<meta name="twitter:title" content="([^"]+?)：無料ゲーム[^"]*"')
  ...
  _re_fields = (
    ...
    ('slogan', re.compile(r'<meta name="description" content="[^"]+」：([^"]+?)"')),
    ('brand', re.compile(r'href="/brand/\d+">([^<]+)<')),
    # Example: ■登録日<br />2015-01-11<br />
    ('date', re.compile(r'■登録日<br />([0-9-]+)<')),
  )
  ...
  def _parsesize(self, h):
    ...
    if m:
      try: return 1024 * int(m.group(1).replace(',', ''))
      except: pass
    return 0
