...
class _Engine(object):
  ...
  def registryLocation():
    ...
    import winreg
    try:
      with winreg.ConnectRegistry(None, winreg.HKEY_LOCAL_MACHINE) as reg: # computer_name = None
        with winreg.OpenKey(reg, REG_PATH) as key:
          path = winreg.QueryValueEx(key, REG_KEY)[0]
...
class Engine(object):
  ...
  def warmup(self, to='en', fr='ja'):
    ...
    try: self.translate("あ", to=to, fr=fr)
