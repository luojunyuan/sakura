...
class ItemApi(object):
  ...
  def query(self, id=None, url=None):
    ...
    if url:
      ...
      if h:
        ...
        if h and 'エラーが発生しました。' not in h:
          ret = self._parse(h)
          if ret:
            ret['id'] = int(id)
            ret['url'] = url
            ret['otome'] = '乙女ゲーム' in h or url.startswith('http://gyutto.me')
            return ret
  ...
  def _parseurlid(self, url):
    ...
    @return  long
    """
    try: return int(self._rx_urlid.search(url).group(1))
    except Exception as e: dwarn(e); return 0
  ...
  def _parse(self, h):
    ...
    if title:
      return {
        ...
        'series': self._parseddlink('シリーズ', h),
        'date': self._parsedate(h),
        'theme': self._parsedd('作品テーマ', h),
        ...
        'artists': list(self._iterparseddlinks('原画', h)) + list(self._iterparseddlinks('原画家', h)),
        'writers': list(self._iterparseddlinks('シナリオ', h)),
        'musicians': list(self._iterparseddlinks('音楽', h)) + list(self._iterparseddlinks('音楽担当', h)),
  ...
  def _parsebrand(self, h):
    ...
    """
    ret = self._parseddlink('ブランド', h) or self._parseddlink('サークル', h)
    return ret.replace(" / ", ',').replace("／", ',') if ret else ''
