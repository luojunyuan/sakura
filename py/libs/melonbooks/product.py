...
class ProductApi(object):
  ...
  def _parse(self, h):
    ...
    if title:
      ret = {
        ...
        'doujin': '<span class="coterie">同人</span>' in h,
        'ecchi': '一般向け' not in h and '18禁' in h,
        'intro': self._parseintro(h),
      }
      desc = self._parsedesc(h)
      if desc:
        ret['date'] = self._parsedesctable(desc, '発行日')
        ret['event'] = self._parsedesctable(desc, 'イベント')

        writers = self._parsecreator(h, 'シナリオ')
        for it in self._iterparsedesctable(desc, 'シナリオ'):
          if it not in writers:
            writers.append(it)
        ret['writers'] = writers # [unicode]

        artists = self._parsecreator(h, '原画')
        for it in self._iterparsedesctable(desc, '作家名'):
  ...
  def _parseintro(self, h):
    ...
    @return  unicode
    """
    ret = skstr.findbetween(h, '<div class="richeditor">', '<!--')
    if ret:
      ret = ret.strip()
    return ret
  ...
  def _parseprice(self, h):
    ...
    @return  int
    """
    try: return int(skstr.findbetween(h, '<td class="price">¥', '<').strip().replace(',', ''))
    except: return 0
