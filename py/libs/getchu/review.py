...
class ReviewApi(object):
  ...
  _rx_review_garbage = re.compile('|'.join((
    '.*?' + re.escape('<!-- *サマリー -->'), # header
    '.*?'.join(( # must be above
      re.escape('<!-- *ソートとページ移動 -->'), # two-paged toolbar, see: http://www.getchu.com/soft.phtml?id=753262&gc=gc
      re.escape('</a>&nbsp;\n</div>'),
    )),
    '.*?'.join(( # must be below
      re.escape('<!-- *ソートとページ移動 -->'), # one-paged toolbar
      re.escape('&nbsp;&nbsp;\n</div>'),
    )),
  )), re.DOTALL)
  def _parse(self, h):
  ...
  _rx_review_btn = re.compile(r'<button .*?レビューを投稿する.*?</button>', re.DOTALL)
  def _clean(self, h):
    """
    @param  h  unicode  HTML
    @return  unicode  HTML
