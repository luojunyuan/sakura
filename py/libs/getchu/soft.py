...
class SoftApi(object):
  ...
  def _parse(self, h):
    ...
    if meta:
      ...
      if title:
        ...
        try: writers = list(filter(bool, meta["シナリオ"].replace("、他", '').split('、')))
        ...
        ret = {
          ...
          'brand': self._fixbrand(meta.get("ブランド") or meta.get("サークル")), # unicode or None
          'subgenres': subgenres, # [unicode]
          'categories': categories, # [unicode]
          'otome': "乙女ゲー" in categories, # bool
          'price': price,                     # int
  ...
  def __maketdrx(name, *args):
    return re.compile(name + r'：</TD><TD align="top">(.+?)</TD>', *args)
  ...
  _rx_td_category = __maketdrx('カテゴリ')
  ...
  _rx_td_subgenre = __maketdrx('サブジャンル\s*', re.DOTALL)
  ...
  def _iterparsesubgenres(self, h):
    ...
    if line:
      for m in self._rx_subgenre.finditer(line):
        ...
        if t and t != '[一覧]' and t != "アドベンチャー" and t != "廉価版・新装版" and t != "その他":
          for it in t.split('・'):
            yield it
  ...
  def _parsecharadesc(self, h):
    ...
    @return  unicode
    """
    START = '<div class="tabletitle">&nbsp;キャラクター</div>'
  ...
  _rx_characters_yomi1 = re.compile(r'）.*')
  _rx_characters_yomi2 = re.compile(r'.*（')
  _rx_characters_cv = re.compile(r'.*：')      # remove leading CV
    ...
    for m in self._rx_characters.finditer(h):
      ...
      if name and name != BAD_NAME:
        ...
        yield {
          ...
          'name': unescapehtml(name).replace('　', ''), # unicode, \u3000
          'yomi': unescapehtml(yomi).replace('　', '') if yomi else '', # unicode
  ...
  def _iterparsecharacters2(self, h):
    ...
    for m in self._rx_characters2.finditer(h):
      ...
      l = list(filter(bool, after.split('<br>')))
      if len(l) == 1:
        label = ''
      elif l:
        label = l[0].strip()
