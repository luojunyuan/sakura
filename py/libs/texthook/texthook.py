...
if skos.WIN:
  ...
  class _TextHook(pytexthook.TextHook):
    ...
    def pruneHookCode(hcode):
      ...
      """
      try:
        h = list(filter(bool, hcode.split()))
        if len(h) == 1:
          return h[0]
  ...
  class TextHook(QObject):
    ...
    dataReceived = Signal(bytearray, bytearray, int, str) # (raw text, rendered text, signature, source)
    hookCodeChanged = Signal(str)
    processAttached = Signal(int) # pid
    processDetached = Signal(int) # pid
    ...
    def setHookCode(self, hcode):
      ...
      if d.pid and d.hcode != hcode:
        if d.hcode:
          ...
          d.hcode = ''
          if d.hooks:
            for name, code in d.hooks.items():
  ...
  class TextHook(QObject): # dummy
    ...
    dataReceived = Signal(bytearray, bytearray, int, str)
    hookCodeChanged = Signal(str)
    processAttached = Signal(int) # pid
    processDetached = Signal(int) # pid
