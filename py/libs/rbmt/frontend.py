...
ANY_LANGUAGE_RE = re.compile(r"^[%s]$" % ANY_LANGUAGE_CHARS)
...
class Lexer:
  ...
  def _tokenize(self, text):
    ...
    phrase = [] # [Token]
    ret = [] # [int link, [Token]]
    for i in range(stream.token_size()):
  ...
  def _parse(self, phrases):
    ...
    if not phrases: # This should only happen at the first iteration
      ...
      for i in range(len(phrases) - 2, -1, -1):
  ...
  def unparse(self, x):
    ...
    if isinstance(x, Token):
      ...
      return self.unparsesep.join(map(self.unparse, x))
