...
class ItemApi(object):
  ...
  def query(self, id):
    ...
    if id and key:
      ...
      if h:
        ...
        if h and '入力頂いた作品IDは既に販売を終了しているか' not in h:
          #print h
          #import sys
          #sys.exit(0)
  ...
  def _parse(self, h, key):
    ...
    if ret:
      ret['otome'] = "成人女性向" in h # bool
      ret['ecchi'] = "18歳未満" in h # bool
      ...
      ret['genre'] = self._parsetablecell("ジャンル", h)
      ret['anime'] = self._hastablecell("アニメ", h)

      ret['artists'] = list(self._iterparsetablelinks('原画', h))
      ret['writers'] = list(self._iterparsetablelinks('シナリオ', h))
      ret['musicians'] = list(self._iterparsetablelinks('音楽', h))
      ret['cv'] = list(self._iterparsetablelinks('声優', h))

      ret['keywords'] = list(self._iterparsetablelinks('キー', h))
      ...
      for it in list(self._iterparsetablelinks('イベント', h)):
        ret['event'] = it
  ...
  _rx_desc_title = re.compile(r'[【『]([^】]*)[】』]')
  #_rx_desc_brand = re.compile(ur'^([^【『]*)[【『]')
  _rx_desc_price = re.compile(r'([0-9,]+) 円') # 3,811 円
  ...
  def _parsebrand(self, h):
    ...
    if m:
      return unescapehtml(m.group(1)).replace(" / ", ',').replace("／", ',')
  ...
  # </tr>
  _rx_date = re.compile(r'(\d{4})年(\d{2})月(\d{2})日')
  def _parsedate(self, h):
    """
    @param  h  unicode  html
    @return  datetime object or None
    """
    start = h.find("登録日</font>")
    if start > 0:
      h = h[start:]
      stop = h.find("</tr>")
      if stop > 0:
  ...
  def _parsedescription(self, h):
    ...
    @return  unicode or None
    """
    start = h.find("の説明</font></strong></div></td>")
    if start > 0:
      stop = h.find("</font></div>", start)
      if stop > 0:
  ...
  def _parsecharadesc(self, h):
    ...
    @return  unicode or None
    """
    start = h.find("キャラクター紹介</strong></font></td>")
    if start > 0:
  ...
  def _parsereview(self, h):
    ...
    @return  unicode or None
    """
    if '本作品の最初のレビューを募集中' in h:
      return
    start = h.find("ユーザーレビュー</font></strong></div></td>")
    if start > 0:
      h = h[start:]
      stop = h.find('<div id="item_history"')
      if stop > 0:
        h = h[:stop]
        DIV = '<div align="left">'
        i = h.find(DIV)
  ...
  _rx_label = re.compile(r"【([^】]+?)】</font>") # <td align="left"><strong><font color="#FF6600">【一人称が僕の女装っ娘】</font></strong><br>
  _rx_cv = re.compile(r"CV：([^）]+?)）")
  _rx_chara = re.compile(r"<strong>●([^<]+?)</strong>") # <strong>●羽馬 紫織（はば・しおり）</strong>
  def _iterparsecharacters(self, h):
    ...
    if m:
      ...
      for i in range(1,100):
        ...
        if m:
          name = unescapehtml(m.group(1)).replace('　', ' ') # u3000
          # ●羽馬 紫織（はば・しおり）
          beg = name.find('（')
          if beg > 0:
            end = name.rfind('）')
            if end > 0:
              yomi = name[beg+1:end].replace('・', ' ')
              name = name[:beg]
