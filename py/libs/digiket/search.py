...
class SearchApi(object):
  ...
  def query(self, text, path, limit=30, sort='new'):
    ...
    if text and path:
      ...
      if h:
        ...
        if h and '<font size="2">0件</font>' not in h:
  ...
  # </div>
  _rx_id_title = re.compile(r'■<a href="/work/show/_data/ID=ITM(\d+?)/">([^<]+?)</a>') # ■<a href="/work/show/_data/ID=ITM0100413/">Sleepover</a></span>
  _rx_price = re.compile(r'(\d+)円')
