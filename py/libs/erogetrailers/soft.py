...
class SoftApi(DataParser):
  ...
  _rx_brands = re.compile(r'<th>ブランド</th><td>(.*?)</td>', re.IGNORECASE)
  ...
  _rx_creators = {    # [(str role, rx))
    'director': __makecreatorsrx("企画・監督"),
    'writer':   __makecreatorsrx("シナリオ"),
    'artist':   __makecreatorsrx("原画"),
    'sdartist': __makecreatorsrx("ＳＤ原画"),
    'musician': __makecreatorsrx("BGM"),
    'musician2':__makecreatorsrx("♫"),
    'singer':   __makecreatorsrx("歌唱"),
    'lyrics':   __makecreatorsrx("作詞"),
    'composer': __makecreatorsrx("作曲"),
    'arranger': __makecreatorsrx("編曲"),
  }
      ...
      for m in rx.finditer(h):
        ...
        for hh in line.split('、'):
          while hh:
            ...
            hh = hh[mm.end(0):]
    except ValueError as e: dwarn(e)
