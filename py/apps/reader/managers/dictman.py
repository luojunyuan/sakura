...
EMPTY_HTML = '<span style="font:youyuan">%s</span>' % (tr_("Not found") + "!＞＜")
...
class _DictionaryManager:
  ...
  def renderHanzi(self, text, html=False):
    ...
    for ch in text:
      if iskanji(ch):
        ...
        info = list(filter(bool, (hanja, trans)))
        if info:
          ch = "%s{%s}" % (ch, ','.join(info))
        text = "【%s】" % ch
        if rad:
  ...
  def _iterEB(self):
    ...
    if ss.isZhongriEnabled():
      ...
      if eb.exists():
        ...
        growl.warn("<br/>".join((
          my.tr("Cannot load {0}").format("Zhongri (日中統合辞典)"),
    ...
    if ss.isWadokuEnabled():
      ...
      if eb.exists():
        ...
        growl.warn("<br/>".join((
          my.tr("Cannot load {0}").format("Wadoku (和独辞書)"),
          my.tr("Please double check its location in Preferences."))))
    ...
    if ss.isDaijirinEnabled():
      ...
      if eb.exists():
        ...
        growl.warn("<br/>".join((
          my.tr("Cannot load {0}").format("DAIJIRIN (大辞林)"),
          my.tr("Please double check its location in Preferences."))))
    ...
    if ss.isKojienEnabled():
      ...
      if eb.exists():
        ...
        growl.warn("<br/>".join((
          my.tr("Cannot load {0}").format("KOJIEN6 (広辞苑)"),
          my.tr("Please double check its location in Preferences."))))
  ...
  _COMPLETE_TRIM_CHARS = 'ぁ', 'ぇ', 'ぃ', 'ぉ', 'ぅ', 'っ', 'ッ'
    ...
    try:
      ...
      if ss.isJishoOrgEnabled() and netman.manager().isOnline():
        ...
        for it in jishomass:
          word = ''
          reading = ''
          if 'word' in list(it['japanese'][0].keys()):
            word = it['japanese'][0]['word']
          if 'reading' in list(it['japanese'][0].keys()):
            reading = it['japanese'][0]['reading']

          jptext = ''
          for jp in it['japanese']:
            if 'word' in list(jp.keys()):
              jptext = jptext + jp['word']
            if 'reading' in list(jp.keys()):
          ...
          for sens in it['senses']:
            definitions = ''
            if 'english_definitions' in list(sens.keys()):
