...
__other_punct = '《》“”‘’"\'，,? 　' # Chinese/English punctuations
_s_punct = jpchars.s_punct + __other_punct + '.()'
_re_punct = re.compile(r"([%s\.\(\)])" % (''.join(jpchars.s_punct) + __other_punct))
def _splittext(text, language):
  ...
  if language.startswith('zh'):
    ...
    return list(filter(bool, _re_punct.split(text))) # split by space and punctuation
