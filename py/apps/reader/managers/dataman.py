...
from PySide2.QtCore import Signal, Slot, Property, Qt, QObject, QDir, QTimer, QCoreApplication, \
    ...
    if g and g.tags: # unicode
      if not ret:
        ...
        for it in g.tags.split(','):
          if it not in ret:
            ret = ','.join((it, ret))
    t = "イチオシ作品," # move it to the beginning
    if t in ret:
      ret = t + ret.replace(t, '')
    ret = ret.replace(',アドベンチャー', '')
    return ret
    ...
    for r in self.getchu, self.freem:
      if r and r.videos:
        for index,vid in enumerate(it for it in r.videos if it not in vids):
          yield {
            'vid': vid,
            'title': "動画 #%s" % (index+1) if index else "動画",
...
class GameObject(QObject):
  ...
  launchPathChanged = Signal(str)
  launchPath = Property(str,
  ...
  brandChanged = Signal(str)
  brand = Property(str,
    lambda self: self.__d.brand,
    notify=brandChanged)

  tagsChanged = Signal(str)
  tags = Property(str,
    lambda self: self.__d.tags,
    notify=tagsChanged)

  fileSizeStringChanged = Signal(str)
  fileSizeString = Property(str,
    lambda self: self.__d.fileSizeString,
    notify=fileSizeStringChanged)

  dateChanged = Signal(int)
  date = Property(int,
...
class Character(QObject):
  ...
  name, nameChanged = __D.synthesize('name', str)
  gender, genderChanged = __D.synthesize('gender', str)
  ttsEngine, ttsEngineChanged = __D.synthesize('ttsEngine', str)
...
class Subtitle(object):
  ...
  def equalText(self, text, exact=False):
    ...
    if self.textName:
      if not exact and text.startswith("「") and text.endswith("」"):
        text = text[1:-1]
      ...
      if not diffcount:
        ...
        if text.startswith("【"):
          return text == ("【%s】%s" % (self.textName, text))
        if text.startswith("「"):
          return text == ("%s「%s」" % (self.textName, text))
      elif diffcount == 4:
        if text.startswith("【") and text.endswith("」"):
  ...
  def equalSub(self, text, exact=False): # unicode, bool -> bool
    ...
    if not exact or not self.subName:
      ...
      if not diffcount:
        ...
        if self.sub.startswith("「") and self.sub.endswith("」"):
          return self.sub[1:-1] == text
      elif not exact and diffcount < 0: # sub is longer
        if self.sub.startswith("【") and self.sub.endswith("」"):
          if text.startswith("「") and text.endswith("」"):
            return self.sub.endswith("】" + text)
          else:
            return self.sub.endswith("】「%s」" % text)
    if self.subName:
      if not exact and text.startswith("「") and text.endswith("」"):
        text = text[1:-1]
      ...
      if not diffcount:
        ...
        if text.startswith("【"):
          return text == ("【%s】%s" % (self.subName, sub))
        if text.startswith("「"):
          return text == ("%s「%s」" % (self.subName, sub))
      elif diffcount == 4:
        if text.startswith("【") and text.endswith("」"):
...
class Comment(QObject):
  ...
  TYPE_NAMES = dict(list(zip(_Comment.TYPES, _Comment.TR_TYPES)))
  ...
  id, idChanged = __D.synthesize('id', int)
  ...
  text, textChanged = __D.synthesize('text', str, sync=True)
  ...
  comment, commentChanged = __D.synthesize('comment', str, sync=True)
  updateComment, updateCommentChanged = __D.synthesize('updateComment', str, sync=True)
  ...
  updateUserNameChanged = Signal(str)
  updateUserName = Property(str,
...
class Term(QObject):
  ...
  TYPES = __D.TYPES
  TR_TYPES = __D.TR_TYPES
  TYPE_NAMES = dict(list(zip(_Term.TYPES, _Term.TR_TYPES)))

  HOSTS = __D.HOSTS
  TR_HOSTS = __D.TR_HOSTS
  HOST_NAMES = dict(list(zip(_Term.HOSTS, _Term.TR_HOSTS)))
  HOST_TYPES = __D.HOST_TYPES

  CONTEXTS = __D.CONTEXTS
  TR_CONTEXTS = __D.TR_CONTEXTS
  CONTEXT_NAMES = dict(list(zip(_Term.CONTEXTS, _Term.TR_CONTEXTS)))
  ...
  role, roleChanged = __D.synthesize('role', str, sync=True)
  pattern, patternChanged = __D.synthesize('pattern', str, sync=True)
  text, textChanged = __D.synthesize('text', str, sync=True)
  ruby, rubyChanged = __D.synthesize('ruby', str, sync=True)
...
class Reference(QObject):
  ...
  TYPES = __D.TYPES
  TR_TYPES = __D.TR_TYPES
  TYPE_NAMES = dict(list(zip(_Reference.TYPES, _Reference.TR_TYPES)))
  ...
  id, idChanged = __D.synthesize('id', int)
  itemId, itemIdChanged = __D.synthesize('itemId', int)
  ...
  title, titleChanged = __D.synthesize('title', str)
  brand, brandChanged = __D.synthesize('brand', str)
  url, urlChanged = __D.synthesize('url', str)
  date, dateChanged = __D.synthesize('date', int)
...
class GetchuReference(Reference): #(object):
  ...
  def iterDescriptions(self):
    ...
    @yield  unicode
    """
    return map(self._renderDescription, self.descriptions)
  ...
  def _renderDescription(cls, t):
    ...
    @return  unicode
    """
    t = cls._rx_desc_title.sub(r'<div class="tabletitle">【\1】</div>', t)
...
class _GameModel(object):
  ...
  def filterData(self):
    if self._filterData is None:
      self._filterData = list(filter(self._searchData, self.sourceData))
...
class GameModel(QAbstractListModel):
  ...
  filterGameTypeChanged = Signal(str)
  filterGameType = Property(str,
...
class _TermModel(object):
  ...
  def filterData(self):
    if self._filterData is None:
      ...
      self._filterData = list(filter(self._searchData, data)) if data else []
    return self._filterData
  @filterData.setter
  def filterData(self, value): self._filterData = value
...
class _DataManager(object):
  ...
  def clearCharacters(self):
    if self.characters:
      for it in self.characters.values():
        if it.parent():
          it.setParent(None)
      self.characters = {}
  ...
  def reloadGameFiles(self):
    ...
    try:
      ...
      self.gameFilesByMd5 = {v.md5:v for v in files.values()}
  ...
  def saveGameFiles(self):
    ...
    ok = rc.jinja_template_write_xml('gamefiles',
      now=datetime.now(),
      files=iter(self.gameFiles.values()),
    )
    if ok:
      ...
      for game in e_games:
        ...
        for e in game:
          ...
          if tag in ('md5', 'path', 'launchPath', 'language', 'launchLanguage', 'loader', 'encoding', 'userDefinedName', 'deletedHook'):
            ...
            g.threadSignature = int(thread.find('./signature').text)
            ...
            if thread is not None:
              ...
              g.nameThreadSignature = int(thread.find('./signature').text)

            threads = e.iterfind('./thread[@type="other"]')
            if threads:
              g.otherThreads = {
                int(e.find('./signature').text) : e.find('./name').text
                for e in threads
              }
          elif tag == 'names':
            l = {
            ...
            try: l['window'] = list(filter(sjis_encodable, l['window']))
            except KeyError: pass
            g.names = l
  ...
  def _updateContexts(self):
    self.contexts = {h:c[0].context for h,c in self.comments.items()} if self.comments else {}
  ...
  def saveGameItems(self):
    ...
    ok = rc.jinja_template_write_xml('gameitems',
      now=datetime.now(),
      items=iter(self.gameItems.values()),
    )
    if ok:
      ...
      for event, elem in context:
        if event == 'start':
          ...
          if path == 3: # grimoire/terms/term
            ...
            terms.append(Term(
                init=False,
                parent=self.q, # FIXME: increase PySide2 reference count
                **kw))
...
class DataManager(QObject):
  ...
  loginChanged = Signal(str, str) # username, password
  ...
  def games(self):
    ...
    @yield  Game
    """
    return iter(self.__d.games.values())
  ...
  def iterContextComments(self):
    ...
    d = self.__d
    if d.comments:
      for h, comments in d.comments.items():
  ...
  def _iterItemGameIds(self, id):
    ...
    @yield  long gameId
    """
    for g in self.__d.gameFiles.values():
      if g.itemId == id:
        yield g.id
  ...
  def _iterSeriesItemIds(self, series):
    ...
    @yield  long itemId
    """
    for g in self.__d.gameItems.values():
      if g.series == series:
        yield g.id
  ...
  def queryItemGame(self, id):
    ...
    @return  Game or None
    """
    for it in self.__d.games.values():
      if it.itemId == id:
        return it
  ...
  def queryGameMd5(self, id):
    ...
    for g in self.__d.games.values():
      if id == g.id:
        return g.md5
  ...
  def updateGameFiles(self):
    if netman.manager().isOnline():
      ...
      #  growl.msg(my.tr("Game database is updated"))
        ...
        try:
          d.gameFilesByMd5 = {v.md5:v for v in files.values()}
          d.gameFiles = files
          d.repairGameItems()
  ...
  def queryGameFile(self, id=0, md5=""):
    ...
    """
    for it in self.__d.gameFiles.values():
  ...
  def characters(self): return list(self.__d.characters.values())
...
class DataManagerProxy(QObject):
  ...
  gameItemIdChanged = Signal(int)
  gameItemId = Property(int,
  ...
  @Slot(str, str, c_longlong, int, result=QObject)
