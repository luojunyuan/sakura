...
class GameProfile(QtCore.QObject):
  ...
  def updateGameNames(self):
    ...
    if path.lower().startswith(PROGRAMFILES.lower()):
      ...
      l = list(filter(bool, path.split('/'))) # filter to skip empty parts
      if l and len(l) <= 3:
...
class GameManager(QtCore.QObject):
  ...
  threadChanged = Signal(int, str) # signature, name, won't emit when game closed
  ...
  hookChanged = Signal(str) # hook, won't emit when game closed
  encodingChanged = Signal(str) # text encoding, won't emit when game closed
  windowChanged = Signal(int) # wid, won't emit when game closed
  processChanged = Signal(int, str) # pid, path, won't emit when game closed

  focusEnabledChanged = Signal(bool)

  nameThreadChanged = Signal(int, str) # signature, name, won't emit when game closed
