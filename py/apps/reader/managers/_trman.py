...
__PARAGRAPH_DELIM = "【】「」…♪" # machine translation of sentence deliminator
...
_re_escape = re.compile(r"^[0-9. 、。？！…]+$")
_re_escape = re.compile(r"(?:Z[A-Y]+Z|[0-9 .,?!%s])+$" % jpchars.s_punct)
...
class TranslationCache:
  ...
  @staticmethod
  def _now(): return int(time()) # -> long  msecs

  def _shrink(self):
    l = sorted(iter(self.data.items()), key=lambda it:it[1][1])
    ...
    if repl:
      try:
        ...
        if repl:
          ...
          repl = repl.replace(" 」", "」") # remove the trailing space
...
class JBeijingTranslator(OfflineMachineTranslator):
  ...
  def createengine():
    ...
    if ok:
      ...
      paths = list(filter(_filter, paths))
      ...
      if ok:
        ...
        dics = '<br/>'.join(map(_tidy, jbeijing.userdic()))
        growl.error('<br/>'.join((
...
class FastAITTranslator(OfflineMachineTranslator):
  ...
  __ja_repl_before = staticmethod(skstr.multireplacer({
    '『': '“‘', # open double single quote
    '』': '’”', # close single double quote
...
class InfoseekTranslator(OnlineMachineTranslator):
  ...
  def translateTest(self, text, to='en', fr='ja', async=False, **kwargs):
    ...
    except Exception as e: dwarn(e); return ''
