...
from PySide2.QtCore import Signal, Slot, Property, QObject, QTimer
...
class _TextManager(object):
  ...
  def updateWindowTranslation(self, verbose=False): # do not growl by default
    ...
    try:
      for h, context in self.windowTexts.items():
    ...
    except Exception as e: # this function could raise if the windowTexts is changed by another thread
      dwarn(e)
...
class TextManager(QObject):
  ...
  textReceived = Signal(str, str, int)   # text, lang, timestamp
  rawTextReceived = Signal(str, str, c_longlong, int)   # text, lang, context hash, context size
  translationReceived = Signal(str, str, str, object, int) # text, language, provider, align, timestamp

  commentReceived = Signal(QObject)  # dataman.Comment
  subtitleReceived = Signal(QObject)  # dataman.SubtitleObject

  nameTextReceived = Signal(str, str)  # text, lang
  nameTranslationReceived = Signal(str, str, str)  # text, lang, provider

  agentTranslationProcessed = Signal(str, str, int, str) # text, hash, role, language
  agentTranslationCancelled = Signal(str, str, int) # text, hash, role
  ...
  def threads(self):
    ...
    @yield  TextThread
    """
    return iter(self.__d.threads.values())

  def removeIgnoredThreads(self):
    d = self.__d
    d.threads = {it.signature:it
        for it in d.threads.values()
...
class TextManagerProxy(QObject):
  ...
  @Slot(str, str, bool, result=bool) # text, type
