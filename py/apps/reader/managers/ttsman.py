...
from PySide2.QtCore import QObject, Slot, QTimer, QMutex
...
class _TtsManager(object):
  ...
  def iterActiveEngines(self):
    ...
    """
    if self._online:
      for it in self._onlineEngines.values():
        if it.isValid():
          yield it
      for it in self._voiceroidEngines.values():
        if it.isValid():
          yield it
      for it in self._voicetextEngines.values():
        if it.isValid():
          yield it
    for it in self._zunkoEngine, self._yukariEngine:
      if it and it.isValid():
        yield it
    for it in self._sapiEngines.values():
      if it.isValid():
        yield it
