...
from PySide2.QtNetwork import QNetworkAccessManager, QNetworkConfigurationManager
        ...
        for e in game:
          ...
          if tag in ('md5', 'encoding', 'language'):
            ...
            if thread is not None and thread.get('deleted') != 'true':
              ...
              ret.threadSignature = int(thread.find('./signature').text)
            thread = e.find('./thread[@type="name"]')
            if thread is not None and thread.get('deleted') != 'true':
              ret.nameThreadName = thread.find('./name').text
              ret.nameThreadSignature = int(thread.find('./signature').text)
          elif tag == 'names':
            ret.names = {
  ...
  def updateGame(self, game, userName, password, deleteHook=False):
    ...
    try:
      ...
      if r.ok and _response_is_xml(r):
        ...
        try: game.itemId = int(e_game.find('./itemId').text)
        except: pass
