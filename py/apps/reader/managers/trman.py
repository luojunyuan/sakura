...
class TranslatorManager(QObject):
  ...
  # Logs
  languagesReceived = Signal(str, str) # fr, to
  normalizedTextReceived = Signal(str) # text after applying translation replacement scripts
  inputTextReceived = Signal(str) # text after applying source terms
  encodedTextReceived = Signal(str) # text after preparing escaped terms
  delegateTextReceived = Signal(str) # text after applying proxied terms
  ...
  jointTranslationReceived = Signal(str)  # translation before applying terms
  delegateTranslationReceived = Signal(str) # text after recovering proxied terms
  decodedTranslationReceived = Signal(str)  # translation after unescaping terms
  outputTranslationReceived = Signal(str)  # translation after applying target terms
  outputSyntacticTranslationReceived = Signal(str)  # translation after applying toutput terms before decoded
