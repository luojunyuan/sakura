...
class TrailersApi(object):
  ...
  def query(self, text=None, key=None, **kwargs):
    ...
    if text and not key and (
        isinstance(text, int) or
        isinstance(text, str) and text.isdigit()
      ):
      key = text
      text = None
  ...
  def _formatItem(cls, kw):
    ...
    try:
      if kw['brand'].endswith('（同人）'):
        kw['doujin'] = True
        kw['brand'] = kw['brand'].replace('（同人）', '')
    except: pass
      ...
      if s:
        for item in s:
          ...
          try: # mono, pcgame, doujin,
            if kw['service_name'] not in ('通販', '美少女ゲーム', '同人'):
              continue
            if kw['floor_name'] == 'アニメ':
              continue
            if '/book/' in kw['url']:
              continue
          ...
          if keywords:
            kw['ecchi'] = not ('一般作品' in keywords or '全年齢向け' in keywords)
            kw['otome'] = "ボーイズラブ" in keywords or '女性向け' in keywords
    ...
    for root in q:
      ...
      for item in items:
        ...
        for a in attrs:
          ...
          try:
            if a == 'ListPrice':
              print("ListPrice =", str(item.ItemAttributes.ListPrice.FormattedPrice), ",", end=' ')
            else:
              print(a, "=", str(getattr(item.ItemAttributes, a)), ",", end=' ')
          except Exception: a, "=,",
        print()
