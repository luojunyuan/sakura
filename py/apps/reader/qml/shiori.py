...
from PySide2.QtCore import Signal, Slot, Property, QObject, QMutex
...
class _ShioriBean:
  ...
  def _renderJson(type, id=0, source='', target='', **kwargs):
    ...
    if type == 'term' and id:
      ...
      l = list(filter(bool, (source, target)))
      return ': '.join(l)
...
class ShioriBean(QObject):
  ...
  popupLookup = Signal(str, str, int, int)  # text, language, x, y
  popupJson = Signal(str, int, int)  # json, x, y
