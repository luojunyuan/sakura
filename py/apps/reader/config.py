...
ENV_PATH = list(map(parse_path, SHARE_YAML['env']['path'])) # [unicode abspath]
ENV_PYTHONPATH = list(map(parse_path, SHARE_YAML['env']['pythonpath'])) # [unicode abspath]
#ENV_PYTHONPATH_XP = map(parse_path, SHARE_YAML['env']['xp']['pythonpath']) # [unicode abspath]

ENV_INTEGRITYPATH = list(map(parse_path, SHARE_YAML['env']['integritypath'])) # [unicode abspath]

ENV_MECABRC = parse_path(SHARE_YAML['env']['mecabrc']) # unicode abspath

#APP_PATH = map(parse_path, APP_YAML['env']['path']) # [unicode abspath]
APP_PYTHONPATH = list(map(parse_path, APP_YAML['env']['pythonpath'])) # [unicode abspath]
...
TR_LOCATIONS = list(map(parse_path, APP_YAML['tr'])) # [unicode abspath]
...
VNRAGENT_DLLS = list(map(parse_path, SHARE_YAML['dlls']['vnragent'])) # [unicode abspath]
VNRHOOK_DLLS = list(map(parse_path, SHARE_YAML['dlls']['vnrhook'])) # [unicode abspath]
VNRBOOT_DLLS = list(map(parse_path, SHARE_YAML['dlls']['vnrboot'])) # [unicode abspath]
...
JCUSERDIC_LOCATIONS = list(map(parse_path, SHARE_YAML['jcuserdic'])) # [unicode abspath]
...
HANVIET_DICS = {k: parse_path(v) # {str key:unicode relpath}
    for k,v in SHARE_YAML['hanviet'].items()}
HANVIET_DIC_LOCATION = os.path.dirname(next(iter(HANVIET_DICS.values())))
