...
class _UserTab(object):
  ...
  def uiLanguageEdit(self):
    ...
    ret.addItems(list(map(config.language_name, config.LANGUAGES)))
  ...
  def userLanguageEdit(self):
    ret = QtWidgets.QComboBox()
    ret.setEditable(False)
    ret.addItems(list(map(i18n.language_name, config.LANGUAGES)))
    ret.setMaxVisibleItems(ret.count())
    ss = settings.global_()
  ...
  def userGenderEdit(self):
    ret = QtWidgets.QComboBox()
    ret.setEditable(False)
    ret.addItems(list(map(i18n.gender_name, defs.GENDERS)))
    def _load():
...
class _TtsTab(object):
  ...
  def testLanguageButton(self):
    ...
    ret.addItems(list(map(i18n.language_name2, config.LANGUAGES2)))
    ret.setMaxVisibleItems(ret.count())
    return ret
  ...
  def yukariButton(self):
    ret = QtWidgets.QRadioButton("結月ゆかり (♀, %s)" % tr_("ja"))
    ret.toggled.connect(self._saveEngine)
    return ret
  @memoizedproperty
  ...
  def zunkoButton(self):
    ret = QtWidgets.QRadioButton("東北ずん子 (♀, %s)" % tr_("ja"))
    ret.toggled.connect(self._saveEngine)
  ...
  def googleButton(self):
    ret = QtWidgets.QRadioButton("Google.com (♀)")
    ret.toggled.connect(self._saveEngine)
    return ret

  @memoizedproperty
  def baiduButton(self):
    ret = QtWidgets.QRadioButton("%s.com (♀, %s)" % (mytr_("Baidu"), tr_("default")))
    ret.toggled.connect(self._saveEngine)
    return ret

  @memoizedproperty
  def naverButton(self):
    ret = QtWidgets.QRadioButton("Naver.com (♀|♂, %s)" % tr_("recommended"))
    ret.toggled.connect(self._saveEngine)
    return ret

  @memoizedproperty
  def bingButton(self):
    ret = QtWidgets.QRadioButton("Bing.com (♀)")
    ret.toggled.connect(self._saveEngine)
    return ret
  ...
  def _saveEngine(self):
    v = ''
    for k,b in self.engineButtons.items():
      if b.isChecked():
        v = k
    dprint("engine = %s" % v)
...
You can specify some keyboard shortcuts in Preferences/Shortcuts."""))
  ...
  def _saveDevice(self):
    for index,btn in self.deviceButtons.items():
      if btn.isChecked():
  ...
  def languageEdit(self):
    ret = QtWidgets.QComboBox()
    ret.setEditable(False)
    ret.addItems(list(map(i18n.language_name2, config.LANGUAGES)))
    ret.setMaxVisibleItems(ret.count())
...
#    return ret
  ...
  def alphabetButton(self):
    ret = QtWidgets.QCheckBox("%s (%s)" % (
        ...
        my.tr("including {0}").format(', '.join(map(i18n.language_name, config.ALPHABET_LANGUAGES)))))
  ...
  def _createRetransLanguageEdit(self, key): # str ->
    ...
    ret.addItems(list(map(i18n.language_name, langs)))
    ret.setMaxVisibleItems(ret.count())
    tm = trman.manager()
  ...
  def romajiTypeEdit(self):
    ...
    ret = QtWidgets.QComboBox()
    ret.setEditable(False)
    ret.addItems(list(map(i18n.ruby_type_name, romajiTypes)))
    ret.setMaxVisibleItems(ret.count())
...
Japanese romanization can be adjusted in the dictionary tab instead."""))
  ...
  def romajaButton(self):
    ret = QtWidgets.QCheckBox(
      "%s, %s: %s" %
      (tr_("Romaja"), my.tr("like this"), "공주님(gongjunim)"))
    ss = settings.global_()
  ...
  def hanjaButton(self):
    ret = QtWidgets.QCheckBox(
      "%s, %s: %s" %
      (tr_("Hanja"), my.tr("like this"), "공주(公主)님"))
    ss = settings.global_()
  ...
  def pinyinButton(self):
    ret = QtWidgets.QRadioButton(
      "%s (%s, %s), %s: %s" %
      (tr_("Pinyin"), mytr_("with tone"), tr_("default"), my.tr("like this"), "麻婆豆腐（má pó dòu fǔ）"))
  ...
  def romajiButton(self):
    ret = QtWidgets.QRadioButton(
      "%s (%s), %s: %s" %
      (tr_("Pinyin"), mytr_("without tone"), my.tr("like this"), "麻婆豆腐（ma po dou fu）"))
    ret.toggled.connect(self._saveChineseGroup)
    return ret
  ...
  def viButton(self):
    ret = QtWidgets.QRadioButton(
      "%s (%s), %s: %s" %
      (tr_("Vietnamese"), "Phiên âm", my.tr("like this"), "麻婆豆腐（ma bà đậu hủ）"))
    ret.toggled.connect(self._saveChineseGroup)
    return ret
...
class _DictionaryTranslationTab(object):
  ...
  def kataButton(self):
    ret = QtWidgets.QRadioButton(
      "%s, %s: %s" %
      (tr_("Katagana"), my.tr("like this"), "可愛い（カワイイ)"))
    ret.toggled.connect(self._saveRubyType)
    return ret
  ...
  def romajiButton(self):
    ret = QtWidgets.QRadioButton(
      "%s, %s: %s" %
      (tr_("Romaji"), my.tr("like this"), "可愛い（kawaii）"))
    ret.toggled.connect(self._saveRubyType)
    return ret
  ...
  def ruButton(self):
    ret = QtWidgets.QRadioButton(
      "%s, %s: %s" %
      (tr_("Russian"), my.tr("like this"), "可愛い（каваий）"))
    ret.toggled.connect(self._saveRubyType)
    return ret
  ...
  def ukButton(self):
    ret = QtWidgets.QRadioButton(
      "%s, %s: %s" %
      (tr_("Ukrainian"), my.tr("like this"), "可愛い（каваії）"))
    ret.toggled.connect(self._saveRubyType)
    return ret
  ...
  def elButton(self):
    ret = QtWidgets.QRadioButton(
      "%s, %s: %s" %
      (tr_("Greek"), my.tr("like this"), "可愛い（καωαιι）"))
    ret.toggled.connect(self._saveRubyType)
    return ret
  ...
  def koButton(self):
    ret = QtWidgets.QRadioButton(
      "%s, %s: %s" %
      (tr_("Korean"), my.tr("like this"), "可愛い（가와이이）"))
    ret.toggled.connect(self._saveRubyType)
    return ret
  ...
  def thButton(self):
    ret = QtWidgets.QRadioButton(
      "%s, %s: %s" %
      (tr_("Thai"), my.tr("like this"), "可愛い（ขะหวะอิอิ）"))
    ret.toggled.connect(self._saveRubyType)
    return ret
  ...
  def arButton(self):
    ret = QtWidgets.QRadioButton(
      "%s, %s: %s" %
      (tr_("Arabic"), my.tr("like this"), "可愛い（كاوايي）"))
    ret.toggled.connect(self._saveRubyType)
    return ret
  ...
  def viButton(self):
    ret = QtWidgets.QRadioButton(
      "%s (%s), %s: %s" %
      (tr_("Vietnamese"), "Phiên âm", my.tr("like this"), "可愛い（khả ái i）"))
    ret.toggled.connect(self._saveRubyType)
    return ret
  ...
  def trButton(self):
    ret = QtWidgets.QRadioButton(
      "%s, %s: %s (%s)" %
      (tr_("English"), my.tr("like this"), "可愛い（cute）",
  ...
  def rubyZhButton(self):
    ret = QtWidgets.QCheckBox("%s, %s: %s (%s)" % (
        tr_("Chinese"), my.tr("like this"), "可愛い（可爱）",
  ...
  def rubyKoButton(self):
    ret = QtWidgets.QCheckBox("%s, %s: %s (%s)" % (
        tr_("Korean"), my.tr("like this"), "可愛い（귀엽다）",
  ...
  def rubyViButton(self):
    ret = QtWidgets.QCheckBox("%s, %s: %s (%s)" % (
        tr_("Vietnamese"), my.tr("like this"), "永遠（vĩnh viễn）",
  ...
  def rubyEnButton(self):
    ret = QtWidgets.QCheckBox("%s, %s: %s (%s)" % (
        tr_("English"), my.tr("like this"), "可愛い（pretty）",
        my.tr("require {0}").format(STARDICT_NAMES['ja-vi'])))
    ret.language = 'en'
    ret.setChecked(ret.language in settings.global_().japaneseRubyLanguages())
    ret.toggled.connect(self._saveRubyLanguage)
  ...
  def rubyDeButton(self):
    ret = QtWidgets.QCheckBox("%s, %s: %s (%s)" % (
        tr_("German"), my.tr("like this"), "可愛い（niedlich）",
  ...
  def rubyFrButton(self):
    lang = 'fr'
    ret = QtWidgets.QCheckBox("%s, %s: %s (%s)" % (
        i18n.language_name(lang), my.tr("like this"), "可愛い（charmant）",
  ...
  def rubyNlButton(self):
    lang = 'nl'
    ret = QtWidgets.QCheckBox("%s, %s: %s (%s)" % (
        i18n.language_name(lang), my.tr("like this"), "可愛い（lief）",
        my.tr("require {0}").format(JMDICT_DICT_NAMES[lang])))
    ret.language = lang
    ret.setChecked(ret.language in settings.global_().japaneseRubyLanguages())
    ret.toggled.connect(self._saveRubyLanguage)
  ...
  def rubyRuButton(self):
    lang = 'ru'
    ret = QtWidgets.QCheckBox("%s, %s: %s (%s)" % (
        i18n.language_name(lang), my.tr("like this"), "可愛い（милый）",
        my.tr("require {0}").format(JMDICT_DICT_NAMES[lang])))
    ret.language = lang
    ret.setChecked(ret.language in settings.global_().japaneseRubyLanguages())
    ret.toggled.connect(self._saveRubyLanguage)
...
class _FontDownloadsTab(object):
  ...
  def fontGroup(self): # Phrase dictionaries
    ...
    r += 1
    for family,info in FONT_INFO.items():
      #if lang == info['lang']:
...
class _TranslatorLibraryTab(object):
  ...
  def jbeijingGroup(self):
    ...
    ret = QtWidgets.QGroupBox(notr_("Kodensha JBeijing") + " (高電社・J北京, 517MB)")
    ret.setLayout(layout)
    return ret
...
+ "<br/>" + my.tr('<span style="color:red">Note: The path cannot contain non-English characters!</span>')
  ...
  def fastaitGroup(self):
    ...
    ret = QtWidgets.QGroupBox(notr_("Kingsoft FastAIT") + " (金山快譯, 534MB)")
    ret.setLayout(layout)
    return ret
...
<center><a href="%s">%s</a></center>""") % (url, url))
  ...
  def dreyeGroup(self):
    ...
    ret = QtWidgets.QGroupBox(notr_("Inventec Dr.eye") + " (英業達・譯典通, 1.35GB)")
    ret.setLayout(layout)
    return ret
...
+ "<br/>" + my.tr('<span style="color:red">TransCAT must be put in a directory with <b>write-permission</b>.</span>')
  ...
  def atlasGroup(self):
    ...
    ret = QtWidgets.QGroupBox(notr_("Fujitsu Atlas") + " (富士通・ATLAS, 597MB)")
    ret.setLayout(layout)
    return ret
...
<center><a href="%s">%s</a></center>""") % urls)
  ...
  def kojienGroup(self):
    ...
    ret = QtWidgets.QGroupBox(notr_("Iwanami Kojien") + " (岩波書店・広辞苑, 3.27GB)")
    ret.setLayout(layout)
    return ret
...
+ "<br/>" +  my.tr('<span style="color:red">Note: EPWING path cannot contain non-English characters!</span>')
  ...
  def daijirinGroup(self):
    ...
    ret = QtWidgets.QGroupBox(notr_("Sanseido Daijirin") + " (三省堂・大辞林, 646MB)")
    ret.setLayout(layout)
    return ret
...
#)
  ...
  def zhongriGroup(self):
    ...
    ret = QtWidgets.QGroupBox(notr_("Shogakukan Zhongri") + " (小学館・日中統合辞典, 30MB)")
    ret.setLayout(layout)
    return ret
...
#  #      self.quickTimeEnableButton.isChecked())
  ...
  def misakiGroup(self):
    ...
    ret = QtWidgets.QGroupBox("VoiceText Misaki (♀, SAPI, 755MB)")
    ret.setLayout(layout)
    return ret
...
<center><a href="%s">%s</a></center>""") % (url, url))
  ...
  def showGroup(self):
    ...
    ret = QtWidgets.QGroupBox("VoiceText Show (♂, SAPI, 724MB)")
    ret.setLayout(layout)
    return ret
...
<center><a href="%s">%s</a></center>""") % (url, url))
  ...
  def yukariGroup(self):
    ...
    ret = QtWidgets.QGroupBox("VOICEROID+ 結月ゆかり (♀, 336MB)")
    ret.setLayout(layout)
    return ret
...
<center><a href="%s">%s</a></center>""") % (url, url))
  ...
  def zunkoGroup(self):
    ...
    ret = QtWidgets.QGroupBox("VOICEROID+ 東北ずん子 (♀, 332MB)")
    ret.setLayout(layout)
    return ret
...
class _EngineTab(object):
  ...
  def scenarioTranslatorButton(self):
    ...
    ret.addItems(list(map(i18n.translator_name, TRANSLATORS)))
    ret.setMaxVisibleItems(ret.count())
    try:
  ...
  def _getTranslators():
    ...
    try:
      ...
      if blans:
        ...
        if 'ru' in blans:
          ret.remove('transru')
    except Exception as e: dwarn(e) # in case I forget to update translator name
    return ret
  ...
  def charSetEdit(self):
    ...
    items = list(map(i18n.win_charset_desc, L))
  ...
  def spaceEncodingEdit(self):
    ...
    items = list(map(i18n.encoding_desc, L))
    items[0] = "(%s)" % tr_("Automatic")
