...
class TextThreadView(QtWidgets.QWidget):
  ...
  threadTypeChanged = Signal(int, int) # type, signature
...
class _TextTab(object):
  ...
  def encodingEdit(self):
    ...
    ret.addItems(list(map(i18n.encoding_desc, config.ENCODINGS)))
  ...
  def _updateThread(self, thread, encoding=None, ignoreType=False):
    ...
    text = '\n\n'.join(map(f, thread.data))
    view.setText(text)

  def _setThreadType(self, type_, signature):
    for sig, view in self._threadViews.items():
      if sig == signature:
...
class TextTab(QtWidgets.QWidget):
  ...
  scenarioThreadChanged = Signal(int, str, str) # signature, name, encoding
  nameThreadChanged = Signal(int, str) # signature, name
