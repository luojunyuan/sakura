...
from PySide2.QtCore import Qt, Signal, Slot, Property, QObject
...
class _TopicInput(object):
  ...
  def scoreRow(self):
    row = QtWidgets.QHBoxLayout()
    for w in self.scoreEdits.values():
      row.addWidget(w)
    row.addStretch()
  ...
  def typeEdit(self):
    ret = QtWidgets.QComboBox()
    ret.setEditable(False)
    ret.addItems(list(map(i18n.topic_type_name, defs.TOPIC_TYPES)))
    ret.setMaxVisibleItems(ret.count())
  ...
  def _save(self):
    ...
    if topic['content'] and topic['title']:
      ...
      if self.topicType == 'review':
        tickets = {}
        for k,v in self.scoreEdits.items():
          score = v.value()
          if score:
            tickets[k] = score
        if tickets:
...
class TopicInput(QtWidgets.QDialog):
  ...
  topicReceived = Signal(str, str, str) # json topic, json image, json tickets
...
class TopicInputManagerBean(QObject):
  ...
  topicReceived = Signal(str, str) # json topic, json image
