...
class DataInfo(object):
  ...
  def __init__(self, data):
    ...
    self.dates = list(map(datetime.fromtimestamp, self.times))
    ...
    #c.reverse() # order asc
    self.userNames = list(map(itemgetter(0), c))
    self.userCounts = list(map(itemgetter(1), c))
    ...
    c = Counter(stat).most_common() # [(k, v)], order by v desc
    #c.reverse() # order asc
    self.languageCounts = list(map(itemgetter(1), c))
    self.languages = list(map(itemgetter(0), c))
    self.languageNames = list(map(i18n.language_name2, self.languages))
