...
if features.WINE:
  ...
  def dump():
    ...
    try:
      with winreg.ConnectRegistry(None, hk) as reg: # computer_name = None
        with winreg.OpenKey(reg, USERDIC_REG_PATH) as path:
          for k in theme.keys():
            try:
              v = winreg.QueryValueEx(path, k)[0]
              print(k, "=", v)
            except WindowsError:
              print(k, "=", None)
    except (WindowsError, TypeError, AttributeError) as e: dwarn(e)
  ...
  def install():
    ...
    try:
      with winreg.ConnectRegistry(None, hk) as reg: # computer_name = None
        with winreg.OpenKey(reg, USERDIC_REG_PATH, winreg.KEY_SET_VALUE) as path:
          for k,v in theme.items():
            winreg.SetValueEx(path, k, 0, winreg.REG_SZ, v)
    except (WindowsError, TypeError, AttributeError) as e: dwarn(e)
  ...
  def uninstall():
    ...
    try:
      with winreg.ConnectRegistry(None, hk) as reg: # computer_name = None
        with winreg.OpenKey(reg, USERDIC_REG_PATH, winreg.KEY_SET_VALUE) as path:
          for k in theme.keys():
            try: winreg.DeleteKeyEx(path, k) # in case the path does not exist
