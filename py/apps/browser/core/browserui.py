...
class _WebBrowser(object):
  ...
  def gestureDirectionNames(self):
    from qtgesture.gesture import MouseGesture
    return {
      MouseGesture.NoMatch: "Φ", # ふぁい
      MouseGesture.Up: "↑", # うえ
      MouseGesture.Down: "↓", # した
      MouseGesture.Left: "←", # ひだり
      MouseGesture.Right: "→", # みぎ
  ...
  def newTabButton(self):
    ...
    ret.setToolTip("%s (%s, %s, %s: →←)" % (i18n.tr("New Tab"), "Ctrl+T", tr_("Double-click"), i18n.tr("Gesture")))
  ...
  def navigationToolBar(self):
    ...
    a = ret.addAction("\u25c0") # left triangle
    a.triggered.connect(self.back)
    a.setToolTip("%s (Ctrl+[, Alt+←, %s: ←)" % (tr_("Back"), i18n.tr("Gesture")))

    a = ret.addAction("\u25B6") # right triangle
    a.triggered.connect(self.forward)
    a.setToolTip("%s (Ctrl+], Alt+→, %s: →)" % (tr_("Forward"), i18n.tr("Gesture")))

    #a = ret.addAction(u'\u27f3') # circle
    a = ret.addAction("◯") # まる
    a.triggered.connect(self.refresh)
    a.setToolTip("%s (Ctrl+R, %s: ↑↓)" % (tr_("Refresh"), i18n.tr("Gesture")))

    #a = ret.addAction(u"\u238c")
    a = ret.addAction("←") # ひだり
    a.triggered.connect(self.undoCloseTab)
    a.setToolTip("%s (Ctrl+Shift+T, %s: ←→)" % (i18n.tr("Undo close tab"), i18n.tr("Gesture")))

    a = ret.addAction("＋") # fullwidth +
    a.triggered.connect(self.zoomIn)
    a.setToolTip("%s (Ctrl+=)" % tr_("Zoom In"))

    a = ret.addAction("ー") # fullwidth -
    a.triggered.connect(self.zoomIn)
    a.setToolTip("%s (Ctrl+-)" % tr_("Zoom Out"))
    return ret
  ...
  def optionToolBar(self):
    ...
    a = self.siteAct = ret.addAction("遊")
    a.setCheckable(True)
    ...
    a = self.annotAct = ret.addAction("訳")
    a.setCheckable(True)
    ...
    a = ret.addAction("≡") # U+226, three lines; alternative: "⌘", U+2318 コマンド記号
