...
ROOT_ABSPATH = os.path.abspath(os.path.dirname(__file__) + "/../../..").replace('\\', '/')
...
def load_yaml_file(path):
  import yaml
  return yaml.load(open(path, 'r', encoding='utf-8'), yaml.FullLoader)
...
def parse_path(p): return p.replace('$PWD', ROOT_LOCATION) # unicode path
...
VERSION_TIMESTAMP = int(APP_YAML['version']['timestamp'])

## Environment variables ##

ENV_PATH = list(map(parse_path, SHARE_YAML['env']['path'])) # [unicode path]
ENV_PYTHONPATH = list(map(parse_path, SHARE_YAML['env']['pythonpath'])) # [str path]
...
TR_LOCATIONS = list(map(parse_path, APP_YAML['tr']['locations'])) # [unicode path]
