...
class YouTubeSolver(QObject):
  ...
  urlSolved = Signal(str, str) # unicode URL, unicode title
...
class _YouTubeSolver(object):
  ...
  def webView(self):
    if not self._webView:
      ...
      self.setWebView(QWebEngineView())
  ...
  def setWebView(self, w):
    ...
    ws.setAttribute(QWebEngineSettings.DnsPrefetchEnabled, True) # better performance
    ws.setAttribute(QWebEngineSettings.PluginsEnabled, False) # disable flash and quicktime
    ws.setAttribute(QWebEngineSettings.AutoLoadImages, False) # do NOT load images
